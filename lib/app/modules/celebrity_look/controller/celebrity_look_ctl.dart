...
//   }
  ...
  final List<String> previousMatches =
      []; // Track previously matched celebrities
  ...
  Future<void> sendImageToGoogleAI(File imgFile) async {
    ...
    // Add previously matched celebrities to the instruction
    String excludedCelebrities = previousMatches.isNotEmpty
        ? "Avoid selecting these celebrities unless necessary: ${previousMatches.join(', ')}."
        : "";
    // String excludedCelebrities = "";

// Select Celebrity Only From this list: $list
    final SystemInstruction =
        '''Analyze the provided image and identify the celebrity that most closely resembles the person in terms of facial structure, hairstyle, features, and overall appearance. Ensure your response is well-rounded by considering subtle differences in expressions, skin tone, and hairstyle. Provide the celebrity name, a confidence score, and a brief explanation for why this celebrity was selected. Avoid repeating the same celebrity unless it is the most accurate match based on all available features.
$excludedCelebrities

Note: 
- Try to avoid most famous celebrities and add randomness in your answer.
- if person in image is bald then always select a celebrity who is bald hair
- if person in image have long beard then always select a personality who is know for long beard, and same goes for short beard and shaved
- Consider same thing for other facial features
- Only Select from $celebrityType 
- Confidance score must be greater then 90%
{
    "name": "<string>",                   // Celebrity's full name
    "country": "<string>",                // Celebrity's country of origin
    "profession": "<string>",             // Celebrity's primary profession
    "match_percentage": "<float>",        // Confidence score between 1 and 100
    "description": "<string>"             // Specific features that align with the celebrity, like face shape, eyes, jawline, eyebrows, etc.
    "otherCelebrities": "<List of String>"             // List all other matched celebrities as well, at least 3
}
''';
    ...
    try {
      ...
      log("jsonMap ${jsonMap}");
      celebrity_match.value = CelebrityMatch.fromJson(jsonMap);
      if (celebrity_match.value != null) {
        previousMatches.add(celebrity_match.value!.celebrityName);
        if (previousMatches.length > 5) {
          previousMatches
              .removeAt(0); // Maintain a rolling history of 5 matches
        }
      }
      imageUrl.value = await APIService()
              .fetchImageUrl(celebrity_match.value!.celebrityName) ??
